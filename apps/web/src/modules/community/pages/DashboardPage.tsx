...
import { useMyCommunities, useCommunities } from "../hooks/useCommunities";
...
const DashboardPage: React.FC = () => {
  ...
  const { data: communitiesData, isLoading: communitiesLoading } =
    useMyCommunities();
  const { data: allCommunitiesData } = useCommunities();
  ...
  const allCommunities = Array.isArray(
    allCommunitiesData?.items ?? allCommunitiesData,
  )
    ? (allCommunitiesData?.items ?? allCommunitiesData)
    : [];
  const notifications = Array.isArray(
    ...
    ? (notificationsData?.items ?? notificationsData)
    : [];
  const tasks = Array.isArray(apiTasks?.items ?? apiTasks)
    ? (apiTasks?.items ?? apiTasks)
    : [];

  const { data: apiEvents } = useQuery({
    queryKey: ["events", "my-communities", communities.map((c: any) => c.id)],
    queryFn: async () => {
      ...
      return results
        ...
        .flatMap((r) =>
          Array.isArray(r.value) ? r.value : (r.value?.items ?? []),
        )
        .sort(
          (a: any, b: any) =>
  ...
  const events = Array.isArray((apiEvents as any)?.items ?? apiEvents)
    ? ((apiEvents as any)?.items ?? apiEvents)
    : [];
  ...
  const tasksDueSoon = activeTasks.slice(0, 5);
  const upcomingEvents = events
    .filter((e: any) => new Date(e.startsAt) > new Date())
    .slice(0, 2);

  // Suggested communities = public communities the user hasn't joined
  const joinedIds = new Set(communities.map((c: any) => c.id));
  const suggestedCommunities = allCommunities
    .filter((c: any) => !joinedIds.has(c.id))
    .slice(0, 3);
    ...
    >
      ...
      <motion.div variants={itemVariants} style={{ marginBottom: 28 }}>
        ...
        <Text style={{ color: "var(--c-text-dim)", fontSize: 15 }}>
          Welcome back, {user?.displayName || "Builder"}. Let's get things
          moving.
        </Text>
      </motion.div>
        ...
        <motion.div variants={itemVariants}>
          <Card
            style={{
              ...
              borderRadius: 16,
              background:
                "linear-gradient(135deg, rgba(124,106,239,0.1) 0%, rgba(54,191,170,0.05) 100%)",
          ...
          >
            <Row align="middle" justify="space-between">
              <Col xs={24} md={16}>
                <Title
                  level={4}
                  style={{ color: "var(--c-text-bright)", margin: 0 }}
                >
                  Getting Started with Commune
                </Title>
                <Paragraph
                  style={{
                ...
                >
                  Follow these steps to unlock the full potential of your
                  community OS.
                </Paragraph>
                <Space size={24} wrap>
          ...
          <Space direction="vertical" size={24} style={{ width: "100%" }}>
            ...
            <motion.div variants={itemVariants}>
              <Card
                title={
                  <Space>
                    <RocketOutlined style={{ color: "var(--c-accent)" }} />
                  ...
                  <List
                    ...
                    renderItem={(t: any) => (
                      ...
                      >
                        <List.Item.Meta
                          title={
                            <Text style={{ color: "var(--c-text-bright)" }}>
                              {t.title}
                            </Text>
                          }
                          description={
                            <Tag
                              color={t.priority === "urgent" ? "red" : "blue"}
                            >
                              {t.priority}
                            </Tag>
                          }
            ...
            <motion.div variants={itemVariants}>
              <Card
                title={
                  <Space>
                    <CompassOutlined style={{ color: "var(--c-info)" }} />
                    <span style={{ color: "var(--c-text-bright)" }}>
                      Suggested Communities
                    </span>
                  </Space>
